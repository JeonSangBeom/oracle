...
-- 뷰 - 테이블에서 분류
-- 인덱스 - 검색 속도 빨리하는 것(데이터 효)
-- 시퀀스 - 순서 메기기
-- PK 프라이머 키 (기준이 되는 키(중복 금지))


--SCOTT 계정이 가지고 있는 정보(감춰진) - DICT
SELECT * FROM DICT; 
...
SELECT * FROM DBA_TABLES;--> 이것 입력하면 뜸
...
--VIEW 추가(보통 사용하지 않는다(뷰는 그냥 가져와서 사용))
...
VALUES ('TEST_ID_01','1234','01-1111-2222');
